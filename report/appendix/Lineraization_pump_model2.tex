...
The control structure chosen in \chapref{control_problem} and \figref{fig:control_structure} includes a PI controller, which is to be designed. The PI controllers purpose is to use the optimized control output from the MPC as a control reference. 
...
The data can be found on the attached storage under the path: \path{CD:/Data/Steady state}.
...
From this the flow through the pumps can be estimated by \eqref{omega_notzero} as the respective pump parameters are found in \appref{system_description} and thus $q$ is the only unknown variable.
...
As it can be seen in \appref{sec:WT_TimeConstant} the dynamic of the system are slow. Therefore will a controller that can settle within 3-5 seconds be sufficiently fast for the system.
