...
With the following operating points set for the pumps, the differential pressure over the pumps and their respective valves, as described in \secref{SystemModel} and seen on \figref{systemdiagram}, are measured. 
The data can be found on the attached storage under the path: \path{CD:/Data/Steady state} and seen on \ref{eq:blablablabla}.
...
From this the operating flow through the pumps can be estimated by \eqref{omega_notzero} as the respective pump parameters are found in \appref{system_description} and thus q is the only unknown variable.
