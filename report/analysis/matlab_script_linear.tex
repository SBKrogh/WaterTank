...
\begin{minipage}[t]{0.20\textwidth}
Where\\
\hspace*{8mm} $\pmb{G} \in \pmb{\mathbb{R}}^{(exu)} $ 
\end{minipage}
\begin{minipage}[t]{0.68\textwidth}
\vspace*{2mm}
...
These two differential equation systems give a full description of the pressures in the whole network, and the effect of the WT on the system. However, due to the linearization the linear system is desired to formulate into the general state-space representation with inputs, outputs and states separated. 
\\
...
\begin{equation}
\Delta \dot{p}_{WT} = - \frac{1}{C_H} \pmb{H^{\dagger}_0}\pmb{H_1}\pmb{q_1} = - \underbrace{\frac{1}{C_H} \pmb{H^+_0}\pmb{H_1}\pmb{{B_1^{T}}}}_\text{\pmb{H}} \pmb{z}
 \label{currentlaw_3}
\end{equation}
