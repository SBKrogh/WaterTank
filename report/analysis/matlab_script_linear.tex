...
As it is shown, there is a differential pressure sensor around the part of the network which is described by the pump model everywhere where a pump is present in the system. Therefore it is unnecessary to use angular velocity as input, the pumps can be operated by the differential pressure as input, since it is known everywhere in the network. 
\\
...
Considering the input vector to the system, the model of the pumps can be written up similarly as the linearized models: 
...
\begin{minipage}[t]{0.68\textwidth}
\vspace*{2mm}
is a matrix representing a linear mapping.
\end{minipage} 
