...
$\pmb{G_p}$ is an extended matrix for the eight inputs which means that the first four columns consist of zeros, since the first four elements of the input vector are the valve opening degrees. $\pmb{G_p}$ can be found in \appref{MappingAppendix}.
...
In \eqref{Amatrix_mod}, the $\pmb{M}$ matrix is invertible for the same reason as it is described in \secref{ParameterEstimation}, for the inertia matrix. The proof of this statement can be found in that section. 
\\
