...
\\
Among these functions, the pipes, valves, the pumps and the edge describing the WT connection are non-linear functions, therefore they need to be linearized. The linearization is carried out according to Taylor-expansion shown in \eqref{TaylorExpansion_approx}.
\\
...
where the partial derivative is a of all first-order partial derivatives of the vectorfield, $\lambda$, in the operating point $\bar{z}$. Since the derivation is according to $\bm{{B_1^{T}}}\bm{z}$, due to the chain rule the derivative is multiplied by $\bm{{B_1^{T}}}$. The reason for only $\lambda(\bm{{B_1^{T}}}\bm{z})$ being expressed is because the elevation is constant and the inertia term is a linear function of the flows.
...
It should be noted here, however, that this control input representation is valid only for the parameter estimation. It is shown in a later chapter, \chapref{control_design}, that for the control, the structure of the model being described here and the input vector is structured differently. However, in this case it is convenient to handle all pumps and valves as input components. Also important to mention that the in the input vector, the differential pressure inputs to the main pumps are taken into account with a negative sign.
\\
...
\begin{equation}
\bm{B_1} \alpha(DP) = \bm{B_1} \bm{G_p} \bm{u}
\label{gamma_lin}
\end{equation}
...
\begin{equation}
 \bm{B}\bm{J {B}}^T \bm{\dot{z}} = \bm{B} g(\bm{B}^T \bm{z})+ \bm{B} u(\bm{\omega},\bm{k_v	}, \bm{B}^T \bm{z})
...
\begin{equation}
 \bm{B}\bm{J {B}}^T \bm{\dot{z}} = \bm{M} \bm{\hat{z}} + \bm{N} \bm{\hat{u}} + \bm{B_o} \Delta \hat{p}_{wt}    
 \label{statespace_1}
\end{equation}
...
\begin{equation}
  \bm{M} = \bm{B_1} \bigg[ \frac{\partial{\lambda(\bm{{B_1^{T}}}\bm{z})}}{{\partial{\bm{{B_1^{T}}}\bm{z}}}}   \bigg]_{\bar{z}} \bm{{B_1^{T}}} +  \bm{B_1} \bigg[ \frac{\partial{\mu(\bm{{B_1^{T}}}\bm{z}, \bm{OD})}}{{\partial{\bm{{B_1^{T}}}\bm{z}}}}  \bigg]_{(\bar{z}, \bar{OD})} \bm{{B_1^{T}}} +  \bm{B_1} \bigg[ \frac{\partial{\gamma(\bm{{B_1^{T}}}\bm{z})}}{{\partial{\bm{{B_1^{T}}}\bm{z}}}}   \bigg]_{\bar{z}} \bm{{B_1^{T}}}
\label{Amatrix}
\end{equation}

And the $\bm{N}$ matrix consists of the following terms:

\begin{equation}
  \bm{N} = \bm{B_1} \bigg[ \frac{\partial{\mu(\bm{{B_1^{T}}}\bm{z}, \bm{OD})}}{{\partial{\bm{u}}}}  \bigg]_{(\bar{z}, \bar{u})} + \bm{B_1}\bm{G_p}  
\label{Bumatrix}
\end{equation}

$\bm{B_o}$ is the cycle matrix belonging to the WT. As can be seen, the linearized terms which are represented in the element-wise model description in \eqref{CompleteModel_extended}, are separated based on if they are multiplied by the small signal values of flows or input. 
...
\begin{equation}
 0 = \bm{M} \bm{\hat{z}} + \bm{N} \bm{\hat{u}} + \bm{B_o} \Delta \hat{p}_{wt}    
 \label{statespace_2}
\end{equation}

The small signal value of the state vector can be expressed on the left side of the equation only if $\bm{M}$ is invertible. In \eqref{Amatrix}, the equation can be rewritten as follows: 

\begin{equation}
  \bm{M} = \bm{B_1}\Bigg[ \bigg[ \frac{\partial{\lambda(\bm{{B_1^{T}}}\bm{z})}}{{\partial{\bm{{B_1^{T}}}\bm{z}}}}   \bigg]_{\bar{z}} +
...
In \eqref{Amatrix_mod}, the $\bm{M}$ matrix is invertible for the same reason as it is described in \secref{ParameterEstimation}, for the inertia matrix. The proof of this statement can be found in that section. 
\\
Expressing the the state vector the following yields:

\begin{equation}
 \bm{\hat{z}} = -(\bm{M^{-1}}\bm{N})\bm{\hat{u}} - (\bm{M^{-1}}\bm{B_o})\Delta \hat{p}_{wt}    
 \label{statespace_2}
\end{equation}
...
\begin{equation}
\Delta \dot{p}_{wt} = (\bm{S}\bm{M^{-1}}\bm{B_o}) \Delta \hat{p}_{WT}  + (\bm{S}\bm{M^{-1}}\bm{N}) \bm{\hat{u}} 
\label{statespace_3}
\end{equation}
...
\begin{equation}
\Delta \dot{p}_{wt} = A_p \Delta \hat{p}_{WT}  + \bm{B_p}\bm{\hat{u}}  
 \label{statespace_4}
\end{equation}
...
\begin{equation}
\bm{\hat{y}} = \bm{C_{1}} \hat{z}  + \bm{C_{2}}\bm{\hat{u}}  
 \label{statespace_5}
\end{equation}
...
\begin{equation}
\bm{\hat{y}} = \bm{C_{1}} (-(\bm{M^{-1}}\bm{N})\bm{\hat{u}} - (\bm{M^{-1}}\bm{B_o})\Delta \hat{p}_{wt})  + \bm{C_{2}}\bm{\hat{u}}  
 \label{statespace_6}
\end{equation}
...
\begin{equation}
\bm{\hat{y}} = \bm{C_{1}} (-(\bm{M^{-1}}\bm{B_o})) \Delta \hat{p}_{wt} + (\bm{C_{1}} (-(\bm{M^{-1}}\bm{N})) + \bm{C_{2}}) \bm{\hat{u}}  
 \label{finaloutput}
\end{equation}
%
\begin{equation}
