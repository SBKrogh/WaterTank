...
\begin{equation}
\label{CompleteModel}
\Delta p_k = \underbrace{\lambda_k (q_k) + \zeta_k + J_k \dot{q_k}}_\text{Pipe} + \underbrace{\mu_k (q_k, k_v)}_\text{Valve} - \underbrace{\alpha_k(\omega_k)}_\text{Pump} + \underbrace{\Delta p_{wt;k}}_\text{Water tank}
\end{equation}
