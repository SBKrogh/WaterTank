...
Similarly to the modeling of the other components, the relation between the two dual variables, pressure difference and flow is derived. The structure of the WT is illustrated in \figref{fig:watertower_sketch}.
...
In \figref{fig:watertower_sketch}, $p_a$ represents the pressure at the surface of the water, thus is it always describing the atmospheric pressure. The variable $p$ is used to describe the pressure value on the bottom of the tank. \\
...
In \eqref{FlowConservation} the volume flow rate, $q$, is equivalent to the current, $I$, in a circuit and the constant term, $\frac{A}{\rho g}$, is equivalent to the capacitance of a capacitor, $C$. The voltage drop is analogous to the pressure drop in the water system.
