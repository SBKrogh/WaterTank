...
In this case small-signal inputs are applied to both the test set up and the system.  Now the linearized model is compared to the real system, therefore the operating point has to be taken into account. Since the linear model is only valid for small deviations in the input around the operating point, the real system has to be excited around the same operating point to obtain the same behavior. In order to achieve this, the operating values are added to bot the input and the output such as: 
