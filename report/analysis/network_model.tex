...
In the same way, the fundamental cycle matrix has been structured such that the last column agrees with the WT edges.
...
The vector containing the pressures at the each node is also re-structured accordantly
...
In \eqref{KCL}, KCL is applied for a connected directed graph, where it is stated that the sum of all flows entering a node must be equal to the sum of flows out of the node.
...
flow through all the nodes. In order to reduce the order of the model and the amount of unknowns, \eqref{ChordRelation} is applied. 
