...
The dynamics of the water distribution system is described. In order to use this linear continuous model subjected to MPC, the model needs to be discretized. The aim is to have a linear discrete-time state-space model with piecewise constant $\Delta \hat{p}_{wt}[k]$, $\bm{\hat{u}}[k]$ and $\bm{\hat{d}}[k]$ signals. The method is chosen as forward Euler-method, which can be formulated as follows:
...
In order to carry out the discretization on the model, the time constant of the dynamics has to be considered which is the time constant of the WT. Recalling the dynamic equation:
...
the time constant can be determined by defining the output and input relation between the WT pressure and the inputs to the main pumps. Since the dynamics are described by a scalar equation, the natural frequency is defined by $A_c$. According to the Nyquist-Shannon sampling theorem,\todo{source}
the sampling frequency must be at least two times higher than the natural frequency of the continues model. Therefore the sampling time is defined such that:
