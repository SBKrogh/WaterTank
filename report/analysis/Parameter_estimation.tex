...
that both models follow different behavior. This might be due to the incapability to excited the system enough so the pipe parameters can be estimated. Moreover, the dynamics of 
the valves are slower in comparison with the pipe's ones, so in the output data the information of the pipes is hidden into the dynamics of the
valves, preventing the estimation process from obtaining the correct values for the valves.


An alternative could be to only use the pumps as inputs without affecting
the valves dynamics, however in order to obtain accurate values for the parameters different escenarios have to be simulated which include varying the water consumption 
of the end-users. 

Therefore, after seeing that the nonlinear approach is not suitable to estimate the unknown parameters, the system will be linearized.