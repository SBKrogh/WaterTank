...
\todo{Explain this better and correct notation!!!}
 \begin{equation}
 \min_{z} \Big(\mathcal{\bm{X}}^T \mathcal{\bm{X}} + \big(\bm{y(B^T z)}_{sys} - \bm{y(B^T z)}_{model}\big)^T  \big(\bm{ y(B^T z)}_{sys} - \bm{ y(B^T z)}_{model}\big)
  \label{ObjectiveFunction}
 \end{equation}
 
\begin{minipage}[t]{0.20\textwidth}
Where\\
\hspace*{8mm} $y(\bm{z})_{sys}$ \\
\hspace*{8mm} $y(\bm{z})_{sys}$ \\
