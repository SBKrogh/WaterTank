...
This chapter explains how the controller is designed in \chapref{Controller} and implemented on the scaled system at Aalborg university.  
...
The convexity of the objective function set up to optimize the power consumption of the pumps, developed through \secref{sec:MPC}, has been verified, in \secref{convexity}. 
...
The PMA pressures also experience a pressure drop, due to the decrease on the control input in the pumps as see in \figref{fig:Output_input}.
...
The input values are no longer in the lower bounds values of the constraints, as seen in \figref{fig:Implementation_shit}, and they vary over time. However, the behavior shown in \figref{fig:Output_input} is not the expected one, since the path followed by the input has nothing to do with the price curve shown in \secref{fig:Implementation_shit}. It would be expected the input to be maximized when the price is low and the opposite with high prices. Considering the unsatisfactory behavior for the input, the same is expected for the state and outputs values. 
