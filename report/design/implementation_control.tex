...
After implementing the MPC, described in \secref{simulink_intro}, multiple complications occurred. The optimization problem deemed convex in \secref{convexity} is, at first, implemented with only the input constraints. This is done to check that the simpler optimization problem can be solved. The input constraints are added because it make no sens to run the pumps outside their physical limits. 
