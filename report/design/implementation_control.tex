...
The controller inputs are no longer in the lower bounds values of the constraints, as seen in \figref{fig:Implementation_shit}, and they vary over time. However, the behavior shown in \figref{fig:Output_input} is not the expected one, since the path followed by the input has nothing to do with the price curve shown in \figref{fig:Implementation_shit}. It would be expected the input to be maximized when the price is low and the opposite with high prices. Considering the unsatisfactory behavior for the input, the same is anticipate for the state and outputs values. 
