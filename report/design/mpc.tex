...
Model predictive control (MPC) is an advanced control method that depends on the dynamic model of the plant. MPC allows to calculate an optimal control signal taking the future costs of some matter into account. The control structure of MPC is in general:
...
To calculate the constraint matrices $H_p$ need to be determined. In \secref{sec:cost_fkt} it can be seen that the price fluctuates a lot but some periodicity can be seen every 24 hours. Furthermore water consumption can be seen as periodic, with a periodicity of 24 hours, due to the daily rhythm of the population. Therefore $H_p$ is chosen to 24. 
