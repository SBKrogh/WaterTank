...
To calculate the constraint matrices $H_p, H_u \text{and} \: H_w$ need to be determined. In \secref{sec:cost_fkt} it can be seen that the price fluctuates a lot but some periodicity can be seen every 24 hours. Furthermore electrical consumption can be seen as periodic, with a periodicity of 24 hours, due to the daily rhythm of the population. Therefor both $H_p$ and $ H_u $ are chosen to 24. $H_w$ is used if deviation from the constraints are allow with in a transit period. Here $H_w$ is chosen to 1, because we want to be within the constraints at all times. 
