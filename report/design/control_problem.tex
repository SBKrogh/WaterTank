...
\begin{align}
 \Upsilon(\bm{u}[k],\bm{q_p}[k],c[k]) &= \frac{1}{\eta} \sum_{i=1}^{H_p-1} \Big( \bm{u^T}[k+i|k] \cdot  \bm{q_{p}}[k+i|k]\Big)\cdot c_p[k+i|k] \label{eqcost} 
\end{align}
...
%
& \underline{y} < \bm{y} < \overline{y}\label{eqcon_output} \\
%
& \underline{\Delta p_{WT}} < \Delta p_{WT} < \overline{\Delta p_{WT}} \label{eqcon_state} \\
%
