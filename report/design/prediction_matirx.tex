...
\\
In order to help the reader, the equation used for reformulating the pump flows is recalled:
...
The operating value for the pump flows can be determined by the non-linear model of the pumps with the available pressure measurements. This is described in detail in \appref{cha:linear_pump}. This means that $\bm{\bar{q}_p}$ yields directly from the pump model. 
\\
...
which shows that the signals has to be decomposed to their constant values in the operating point and the small- signal deviations. 
...
Thus at time step $k$, these signals consist of the present and future input and disturbance vectors until the end of the horizon, which is $H_p - 1$ since $i$ is iterated from zero. In the further discussion the same notation is used for signals referring to the whole prediction horizon as introduced in \eqref{eq:uhp} and \eqref{eq:dhp}.
...
\begin{aligned} 
 ...
 & + \underbrace{({\bm{\bar{u}}}_{\bm{{Hp}}} + {\bm{\hat{u}}}_{\bm{{Hp}}})^{T}{\bm{\Lambda}}_{\bm{3,Hp}}\bm{\Phi} \bm{\Delta \hat{p}_{\bm{wt,Hp}}}[0]}_{\RN{4}}+ \underbrace{({\bm{\bar{u}}}_{\bm{{Hp}}} + {\bm{\hat{u}}}_{\bm{{Hp}}})^{T}{\bm{\Lambda}}_{\bm{3,Hp}}\bm{\Gamma} \bm{\hat{u}}_{\bm{Hp}}}_{\RN{5}} + \underbrace{({\bm{\bar{u}}}_{\bm{{Hp}}} + {\bm{\hat{u}}}_{\bm{{Hp}}})^{T}{\bm{\Lambda}}_{\bm{3,Hp}}\bm{\Psi}\bm{\hat{d}}_{\bm{Hp}}}_{\RN{6}} \bigg]
 \end{aligned}
\end{equation}
...
\begin{equation}
\underset{\bm{\hat{u}_{Hp}}}{min} \:  \Upsilon(\cdot) = \underset{\bm{\hat{u}_{Hp}}}{min} \:  \frac{1}{\eta}\bigg( \frac{1}{2} \bm{\hat{u}}_{\bm{Hp}}^{T} \bm{R} \bm{\hat{u}}_{\bm{Hp}} + \bm{b} \bm{\hat{u}}_{\bm{Hp}} + \bm{c} \bigg)
\end{equation}